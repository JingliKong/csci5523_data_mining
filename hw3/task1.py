...
def main(input_file, output_file, jac_thr, n_bands, n_rows, sc):
    ...
    signatures = pairs.map(lambda x: MinHash(x, args.n_bands * args.n_rows))
    ...
    # TODO: create SORT then create combination pairs inside each bucket drop the bucket_id and these are all the candidate pairs so now we have ((b_id, partial_sig), (b_id, partial_sig))
    # TODO: after returning the combinations we have to do .distinct to make sure we dont have duplicate pairs    
    # TODO: then apply the jaccard similarity between pairs on the partial sig and filter it based on the threshold 
    # TODO: its greater return a pair like (b_id, b_id, jaccard simularity) after removing the partial sigs
    # TODO: Then make sure to write the tuples to the json file in the correct format   
